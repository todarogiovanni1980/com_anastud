...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_anastud/assets/css/anastud.css');
$document->addStyleSheet(JUri::root() . 'media/com_anastud/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_anastud');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_anastud&task=studenti.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'studenteList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
		...
		<div id="j-main-container">
			...
			<table class="table table-striped" id="studenteList">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canCreate  = $user->authorise('core.create', 'com_anastud');
					$canEdit    = $user->authorise('core.edit', 'com_anastud');
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'studenti.', $canChange, 'cb'); ?>
</td>
						<?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_anastud&task=studente.edit&id='.(int) $item->id); ?>">
					<?php echo $this->escape($item->cognome); ?></a>
				<?php else : ?>
					<?php echo $this->escape($item->cognome); ?>
				<?php endif; ?>
					...
					<?php
						if (!empty($item->foto)) :
							$fotoArr = explode(',', $item->foto);
							foreach ($fotoArr as $fileSingle) :
								if (!is_array($fileSingle)) :
									$uploadPath = '/fotostudenti' .DIRECTORY_SEPARATOR . $fileSingle;
									echo '<a href="' . JRoute::_(JUri::root() . $uploadPath, false) . '" target="_blank" title="See the foto">' . $fileSingle . '</a> | ';
								endif;
							endforeach;
						else:
							echo $item->foto;
